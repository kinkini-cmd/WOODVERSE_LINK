// Compatibility barrel for existing imports. New routes import by service.
export {
  HomePage,
  SellerPage,
} from "../services/home";
...
export { ChatbotPage } from "../services/ai";
export {
  LoginPage,
  ForgotPasswordPage,
  ProfilePage,
} from "../services/auth";import { useMemo, useState } from "react";
import {
  AlertTriangle,
  ArrowLeft,
  ArrowRight,
...
function HomePage({ addToCart }) {
  ...
  return (
    <main className="overflow-hidden bg-[#f7f8f5] text-[#17231f] dark:bg-[#101714] dark:text-stone-100">
      ...
      <section id="how-it-works" className="page-shell py-24">
        <SectionHeading align="center" title="How WoodVerse works" subtitle="A simple flow that keeps every handoff visible." />
        <div className="relative grid gap-10 md:grid-cols-4">
          {[[Search, "1", "Discover", "Customers find the right product or material."], [ClipboardList, "2", "Coordinate", "Vendors review stock and supplier availability."], [Factory, "3", "Create", "Approved manufacturing work becomes production tracking."], [Truck, "4", "Deliver", "Shipments move to the customer with clear status updates."]].map(([Icon, number, title, detail], index, steps) => (
            <article key={number} className="relative flex flex-col items-center text-center">
              <div className="relative flex h-16 w-full items-center justify-center">
                <span className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 text-5xl font-black leading-none text-[#dbeee3] dark:text-[#29463a]">{number}</span>
                <span className="relative z-10 grid h-16 w-16 place-items-center rounded-2xl bg-[#1c614f] text-white shadow-lg">
                  <Icon className="h-7 w-7" />
                </span>
                {index < steps.length - 1 && <span className="absolute left-[calc(50%+2rem)] top-8 hidden h-px w-[calc(100%-2rem)] bg-[#bdd6c7] md:block" aria-hidden="true" />}
              </div>
              <h3 className="mt-5 text-xl font-extrabold">{title}</h3>
              <p className="mt-2 max-w-[15rem] text-sm leading-relaxed text-[#65736c] dark:text-stone-300">{detail}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <footer className="bg-[#102f27] py-14 text-white"><div className="page-shell grid gap-10 md:grid-cols-[1.3fr_1fr_1fr_1fr]"><div><div className="flex items-center gap-3"><img src="/assets/woodverse-logo.png" alt="WoodVerse" className="h-10 w-10 rounded-lg" /><strong className="text-xl">WoodVerse</strong></div><p className="mt-4 max-w-xs text-sm leading-relaxed text-emerald-100/65">The connected platform for customers, vendors, and suppliers in woodcraft.</p></div>{[["Platform", "About", "Features", "Pricing", "Contact"], ["Resources", "FAQ", "Blog", "Careers", "Help Center"], ["Legal", "Privacy Policy", "Terms & Conditions", "Cookie Policy"]].map(([title, ...links]) => <div key={title}><h3 className="font-extrabold text-[#d8a36b]">{title}</h3><div className="mt-4 grid gap-3 text-sm text-emerald-100/65">{links.map((link) => <button key={link} onClick={() => { const target = `#${link.toLowerCase().replaceAll(" ", "-")}`; document.querySelector(target)?.scrollIntoView({ behavior: "smooth" }); }} className="text-left transition hover:text-white">{link}</button>)}</div></div>)}</div><div className="page-shell mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-white/15 pt-6 text-xs text-emerald-100/55"><span>© 2026 WoodVerse. All rights reserved.</span><span>Made for sustainable Sri Lankan craftsmanship.</span></div></footer>
