  ...
  return (
    <footer className="bg-white dark:bg-[#202624]">
      <div className="page-shell grid grid-cols-[1.8fr_.75fr_.85fr_1.7fr] gap-10 py-14 lg:gap-16 max-lg:grid-cols-2 max-sm:grid-cols-1">
        <div>
          <button onClick={() => navigate("/")} className="text-left font-extrabold text-forest dark:text-emerald-200">WoodVerse</button>
        ...
        <FooterLinks title="Marketplace" links={["New Arrivals", "Top Vendors", "Timber Guides", "Special Offers"]} />
        <FooterLinks title="Materials" links={["Teak Sourcing", "Mahogany Standards", "Forestry Policy", "Durability Testing"]} />
        <div>
          ...
          <form className="mt-6 grid grid-cols-[minmax(0,1fr)_auto] gap-2 max-sm:grid-cols-1">
            <input className="min-w-0 rounded-md border border-slate-200 bg-blue-50 px-3 py-2 outline-none dark:border-slate-700 dark:bg-[#1d2422]" placeholder="Your email address" />
            <button className="rounded-md bg-forest px-5 py-2 text-white">Join</button>
          </form>
        </div>
      </div>
      <div className="page-shell flex justify-between gap-6 border-t border-slate-200 py-8 text-sm text-slate-500 dark:border-slate-700 dark:text-stone-400 max-sm:flex-col">
        ...
        <nav className="flex flex-wrap gap-x-8 gap-y-3"><a>Sustainability</a><a>Vendor Policy</a><a>Privacy</a></nav>
      </div>
    </footer>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-3 text-slate-500 dark:text-stone-400">
        {links.map((link) => <a key={link} className="break-words leading-snug">{link}</a>)}
      </div>
    </div>
  );
}
