      ...
      <nav className="sticky top-0 z-40 border-b border-white/20 bg-[#102f27]/90 text-white shadow-lg backdrop-blur-xl">
        <div className="page-shell flex min-h-20 items-center justify-between gap-6">
          ...
          <div className={`${mobileMenuOpen ? "absolute left-4 right-4 top-[76px] grid rounded-xl border border-white/10 bg-[#102f27] p-4 shadow-2xl" : "hidden"} items-center gap-1 lg:static lg:flex lg:bg-transparent lg:p-0 lg:shadow-none`}>
            {["Home", "About", "Services", "Features", "How It Works", "Pricing", "FAQ", "Contact"].map((item) => <button key={item} onClick={() => scrollTo(`#${item === "Home" ? "top" : item.toLowerCase().replaceAll(" ", "-")}`)} className="rounded-lg px-3 py-2 text-left text-sm font-bold text-emerald-50/80 transition hover:bg-white/10 hover:text-white">{item}</button>)}
  ...
  const send = async (text) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/ai/chat`, {
        ...
        body: JSON.stringify({ message, context: { role: "customer", page: "chatbot" } }),
      });
        ...
        <form
          onSubmit={async (event) => {
            ...
            if (isRegister) {
              ...
              try {
                const result = await apiRequest("/api/users", { method: "POST", body: JSON.stringify({ email: registeredEmail, fullName: `${formData.get("firstName")} ${formData.get("lastName")}`.trim(), role: accountType }) });
