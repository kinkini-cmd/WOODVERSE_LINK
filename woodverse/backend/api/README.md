...
Load the demo database records. This resets the demo tables and inserts 20 rows into each table in `../../database/schema.sql`.
...
- `src/server.js` - Entry point, Express app, Socket.IO server
- `src/routes/` - Modular route handlers by resource
- `src/utils/` - Shared server utilities and helpers
- `src/data/` - In-memory fallback data
- `src/socket.js` - Socket.IO event handlers
- `src/db.js` - PostgreSQL connection and query helper
